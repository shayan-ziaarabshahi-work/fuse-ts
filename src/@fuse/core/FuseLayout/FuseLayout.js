...
import { initialStacks } from 'app/store/stackNavigationSlice';
import { useFuseDispatch, useFuseSelector } from 'src/app/utils/hooks/useStore';
import FuseSplashScreen from '../FuseSplashScreen/FuseSplashScreen';
...
function FuseLayout(props) {
  ...
  const settings = useFuseSelector(selectFuseCurrentSettings);
  const defaultSettings = useFuseSelector(selectFuseDefaultSettings);
  ...
  const matched = matchedRoutes?.find((route) => route.pathnameBase === formattedPathname) || false;
  ...
  useEffect(() => {
    dispatch(initialStacks({ routes, pathname }));
